...
import { Lock, Heart, Shield, ArrowLeft, AlertCircle } from 'lucide-react';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onSuccess, onBackToCalculator }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.warn("Google Sign-In Popup failed:", err);
      if (err.code === 'auth/popup-blocked' || err.code === 'auth/popup-closed-by-user') {
        setError("Sign-in popup was closed or blocked by the browser. Please tap below to try again.");
    ...
    <div className="flex flex-col min-h-screen max-w-md mx-auto bg-slate-950 text-slate-100 p-6 justify-between select-none">
      ...
      <div className="my-auto py-8 text-center flex flex-col items-center">
        ...
        <p className="text-slate-400 text-sm max-w-xs leading-relaxed">
          Private, encrypted real-time communications for two souls. Sign in with your Google account to connect.
        </p>
      ...
      <div className="text-center text-[11px] text-slate-500 pb-2">
        Connected to Firebase Realtime Database: chat-7305d
      </div>
    </div>
  );
};
